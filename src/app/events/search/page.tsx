...
function SearchResultForEvent() {
  ...
  const requestBody = searchData.destination && searchData.checkIn && searchData.checkOut && searchData.eventType ? {
    eventType: searchData.eventType,
    destination: searchData.destination,
    guestCount: parseInt(searchData.adults) || 0,
    eventStartDate: parseDate(searchData.checkIn),
    eventEndDate: parseDate(searchData.checkOut),
  } : null;

  const { data: events = [], isLoading, error: errorObject } = useQuery({
    queryKey: ['searchEvents', requestBody],
    queryFn: async () => {
      if (!requestBody) return [];
      const response = await apiClient.post(API_ENDPOINTS.SEARCH_EVENTS, requestBody);
      const result = response.data;
      if (result.status === 200 && Array.isArray(result.data)) {
        return result.data.map((item: ApiEventResponse) => ({
  ...
  const filteredEvents = events.filter((event: { pricePerEvent: number; type: string; amenities: any[]; }) => {
